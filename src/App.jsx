...
export default function App() {
  ...
  useEffect(() => {
    ...
    crt.style.cssText = `position:fixed;inset:0;pointer-events:none;z-index:99999;background:repeating-linear-gradient(0deg,rgba(0,0,0,0.025) 0px,rgba(0,0,0,0.025) 1px,transparent 1px,transparent 3px);`;
    const vig = document.createElement('div');
    vig.style.cssText = `position:fixed;inset:0;pointer-events:none;z-index:99998;background:radial-gradient(ellipse at center,transparent 55%,rgba(0,0,0,0.5) 100%);`;
  ...
  const autoMaximize = useCallback((id) => {
    if (isMobile()) {
      setTimeout(() => {
        const deskH = window.innerHeight - 34;
  ...
  const handleOpen = useCallback((id) => {
    ...
    openWindow(id);
    autoMaximize(id);
  }, [openWindow, showMsg, autoMaximize]);
  ...
  }, [windows, openWindow, minimizeWindow, autoMaximize]);
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ height: 28, background: '#c0c0c0', borderBottom: '1px solid #808080', display: 'flex', alignItems: 'center', gap: 4, padding: '2px 6px', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }} />
        <div style={{ height: 20, background: '#fff', borderTop: '1px solid #808080', borderLeft: '1px solid #808080', padding: '0 8px', fontSize: 11, fontFamily: "'Share Tech Mono'", display: 'flex', alignItems: 'center', minWidth: 120 }}>
          C:\MARIEM
        </div>
      </div>
      <div style={{ flex: 1, background: '#fff', padding: 12, display: 'flex', flexWrap: 'wrap', gap: 10, alignContent: 'flex-start', overflowY: 'auto' }}>
        {items.map(item => (
          <div
            ...
            style={{ width: 72, textAlign: 'center', padding: 4, cursor: 'pointer' }}
            onMouseEnter={e => e.currentTarget.style.outline = '1px dotted #808080'}
            onMouseLeave={e => e.currentTarget.style.outline = 'none'}
          >
            <div style={{ imageRendering: 'pixelated', display: 'flex', justifyContent: 'center', width: 34, height: 34, margin: '0 auto 2px' }}>
              <ExItemIcon id={item.id}/>
            </div>
    ...
    >
      <div style={{ padding: 16, fontFamily: "'Share Tech Mono'", fontSize: 12, overflowY: 'auto', height: '100%' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <button onClick={() => closeWindow(id)} style={{ padding: '4px 28px', background: '#c0c0c0', borderTop: '2px solid #fff', borderLeft: '2px solid #fff', borderRight: '2px solid #404040', borderBottom: '2px solid #404040', fontFamily: "'Share Tech Mono'", fontSize: 12 }}>OK</button>
        </div>
      </div>
    </Win95Window>
  );
